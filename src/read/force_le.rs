...
/// Extension trait for `Read` that provides methods for reading supported value types in little endian order without the `Result` return type, triggering a panic on error.
///
/// **Note:** do not borrow this as `&mut dyn ForceReadValLe`, as this would not compile. Use `&mut dyn Read` instead.
pub trait ForceReadValLe: Read {
    auto_impl_nofns!();
...
impl<T: Read> ForceReadValLe for T {}
