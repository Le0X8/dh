...
fn read_primitive() {
    ...
    let val: u8 = cursor.read_be().unwrap();
    assert_eq!(val, 0);

    let val: [u8; 3] = cursor.read_be().unwrap();
    assert_eq!(val, [1, 2, 3]);
    ...
    let val: u16 = cursor.read_be().unwrap();
    assert_eq!(val, 0x06_07);

    // overflow
    let val: Result<u8> = cursor.read_be();
    assert!(val.is_err());
}
...
fn read_u32_be() {
    ...
    assert_eq!(cursor.read_u32().unwrap(), 0x9ABCDEF0);
...
fn read_u64_be() {
    ...
    assert_eq!(cursor.read_u64().unwrap(), 0x0123456789ABCDEF);
